...
type DialogMode = 'create' | 'edit' | 'customers' | 'delete' | null;
...
export function DistributorManagement() {
  ...
  const openDelete = (d: Distributor) => {
    setTarget(d);
    setFormError(null);
    setDialog('delete');
  };
  ...
  const onConfirmDelete = async () => {
    ...
    setFormError(null);
    try {
      await DistributorService.delete(target.id);
      closeDialog();
      await load();
    } catch (err) {
        ...
        >
          <div
            style={{
              ...
              borderRadius: 12,
              width: '100%',
              maxWidth: dialog === 'customers' ? 520 : 440,
              padding: 24,
          ...
          >
            ...
            >
              <h2 style={{ margin: 0, fontSize: '1.125rem', fontWeight: 700, color: '#111827' }}>
                ...
                {dialog === 'delete' && 'Delete Distributor'}
              </h2>
              <button
                type="button"
                onClick={closeDialog}
            ...
            {dialog === 'delete' && (
              <div>
                ...
                >
                  Delete <strong>{target?.company}</strong>? This removes the distributor and
                  related reports/sales from active lists. This cannot be undone from the UI.
                </p>
